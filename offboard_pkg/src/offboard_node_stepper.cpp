...
int main(int argc, char **argv)
{
	...
	float dfy = target_position.pose.position.y-current_position.pose.position.y;
        //target_position.pose.position.x = target_position.pose.position.x - 5*sin(atan(dfy/dfx));
        //target_position.pose.position.y = target_position.pose.position.y - 5*cos(atan(dfy/dfx));
	...
	if((abs(current_position.pose.position.x-pose.pose.position.x)<0.5f)&&(abs(current_position.pose.position.y-pose.pose.position.y)<0.5f)&&(abs(current_position.pose.position.y-pose.pose.position.y)<0.5f))
        {
            ...
            pose.pose.position.x += flight_dist*cos(atan(dfy/dfx));
        ...
        ROS_INFO("pub x: [%f]", pose.pose.position.x);
        ROS_INFO("pub y: [%f]", pose.pose.position.y);
