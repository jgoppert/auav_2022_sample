...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()){
        ...
        float dfx = current_position.pose.position.x-pose.pose.position.x;
        float dfy = current_position.pose.position.y-pose.pose.position.y;
        current_position.pose.position.x = current_position.pose.position.x - 5*sin(atan(dfy/dfx));
        current_position.pose.position.y = current_position.pose.position.y - 5*cos(atan(dfy/dfx));
        ...
        if((abs(current_position.pose.position.x-pose.pose.position.x)<0.5f)&&(abs(current_position.pose.position.y-pose.pose.position.y)<0.5f)&&(abs(current_position.pose.position.y-pose.pose.position.y)<0.5f))
            {
                ...
                //pose.pose.position.z = 3;
            pose.pose.position.x = current_position.pose.position.y;
            pose.pose.position.y = current_position.pose.position.x;
            pose.pose.position.z = 3;
            local_pos_pub.publish(pose);
